...
import { CouponEnum, OrderStatusEnum, PaymentEnumType, PaymentService } from 'src/common';
...
export class OrderService {
  ...
  async webhook(req:Request){
    ...
    if(!order){
      throw new BadRequestException("Failed to find matching order")
    }
  ...
  async create(createOrderDto: CreateOrderDto,user:UserDocument):Promise<OrderDocument> {
    ...
    if(createOrderDto.coupon){
      ...
      if(!coupon){
        throw new NotFoundException("Failed to find matching coupon")
      }
      if(coupon.duration <= coupon.usedBy.filter((ele)=>{return ele.toString()==user._id.toString()}).length){
        throw new ConflictException(`You have reached the limit for this coupon. It can be used only ${coupon.duration} times for each user `)
      }
    }
    let total=0
    ...
    for (const product of cart.products){
      ...
      if(!cartProduct){
        throw new NotFoundException(`Failed to find matching product ${product.productId} or out of stock `)
      }
...
delete createOrderDto.coupon
    ...
    if(!order){
      throw new BadRequestException("Failed to create this order")
    }
    if(coupon){
