...
createLoginCredentials = async (user:UserDocument):Promise<LoginCredentialsResponse>=>{
    ...
    const access_token= await this.generateToken({
        ...
        options:{expiresIn:Number(process.env.ACCESS_TOKEN_EXPIRES_IN),jwtid,secret:signatures.access_signature,}
    })
    const refresh_token= await this.generateToken({
        payload:{_id:user._id},
        
        options:{expiresIn:Number(process.env.REFRESH_TOKEN_EXPIRES_IN),jwtid,secret: signatures.refresh_signature,}
    })
